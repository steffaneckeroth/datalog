...
if($db->connect()) {
	$data = array();
	$from = strtotime("YESTERDAY MIDNIGHT") - 300;
	$to = strtotime("TODAY MIDNIGHT") + 300;
	...
	foreach($data as $building => $key_timestamp) {
		//both pdfs
		$header = '<table width="100%" style="font-weight:bold;vertical-align:top;"><tr><td width="50%"><img src="/var/www/html/data-exporter/sirena.png" style="height: 25px;" /></td><td width="50%" style="font-size:20px;text-align:right;">Temperatur rapport<br/>' . $building . ' ' . date("d-m-Y", (time() - 86400)) . '</td></tr></table>';
		...
		//print table pdf
		exec("/usr/bin/lp '" . $file_path . $file_name . "_table.pdf'");
	}
